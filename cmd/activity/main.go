...
import (
	...
	activitysvc "github.com/ca-risken/aws/pkg/activity"
	grpcclient "github.com/ca-risken/aws/pkg/grpc"
...
func main() {
	...
	awsc, err := grpcclient.NewAWSClient(ctx, conf.DataSourceAPISvcAddr)
	if err != nil {
		appLogger.Fatalf(ctx, "Failed to create aws client: %+v", err)
	}
	service := activitysvc.NewActivityService(awsc, conf.AWSRegion, appLogger)
